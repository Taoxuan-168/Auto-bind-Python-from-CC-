...
# distclean    
    ...
    if os.path.exists(global_config_file):
        # 创建临时备份（使用系统临时目录 + 唯一名称）
        import tempfile
        global_config_backup = tempfile.NamedTemporaryFile(mode='wb', delete=False)
        with open(global_config_file, 'rb') as f_src, global_config_backup as f_dst:
            f_dst.write(f_src.read())
        global_config_backup_path = global_config_backup.name
        print(f"-- Backup {global_config_file} to {global_config_backup_path}")
    ...
    # 3. 恢复 global_config.mk（如果有备份）
    if global_config_backup_path:
        # 重建 build/config 目录
        os.makedirs(os.path.dirname(global_config_file), exist_ok=True)
        # 恢复文件
        with open(global_config_backup_path, 'rb') as f_src, open(global_config_file, 'wb') as f_dst:
            f_dst.write(f_src.read())
        # 删除临时备份
        os.unlink(global_config_backup_path)
        print(f"-- Restore {global_config_file} from backup")
